  ...
  const verificationOptions = [
    {
      value: "0",
      label:
        "よりよく理解してもらうため、荘司さんがさらに深堀する質問を考えてそれへの回答で還元情報を補足します。",
    },
    {
      value: "1",
      label:
        "正確に質問に回答するために補足すると役に立つ情報を引き出す質問をして回答をまとめます。",
    },
  ];
  ...
  const sendMessageToGemini = async (message, questions) => {
    ...
    try {
      if (
        ...
        const answer = await axios.post("http://localhost:8002/api/gpt", {
          message: message,
        });
        if (
          selectedResponder === "荘司さん（会話しながら回答）" &&
          ...
          const geminiCreateQuestionAndAnswer = [
            ...
            { role: "assistant", content: answer.data },
          ];
          //geminiが作った質問と回答を更新
          ...
          const send_message = answer.data;
          console.log(send_message);
          setAnswer(send_message); // 回答を state に設定
          setQuestion(message); // 質問を state に設定
          setResponder(selectedResponder);
          setVerification(selectedVerification);
          navigate("/answer");
    ...
    try {
      ...
      if (
        ...
        while (score_count < 3) {
          ...
          `;
          const presentQuestion = await axios.post(
            "http://localhost:8002/api/gpt",
            {
              message: first_prompt,
            }
          );

          const jsonString = presentQuestion.data.match(/\{.*\}/s);
          ...
          if (jsonString) {
            ...
            const response = await axios.post("http://localhost:8001/api/llm", {
            ...
            if (similarity_score > 3.0) {
              ...
              //geminiが作った質問に対する回答
              const presentAnswer = await axios.post(
                "http://localhost:8002/api/gpt",
                {
                  message: preset_question_prompt,
                }
              );
              ...
              const geminiCreateQuestionAndAnswer = [
                ...
                { role: "assistant", content: presentAnswer.data },
              ];
        ...
        "${questionText}"
      `;
        const base_answer = await axios.post("http://localhost:8002/api/gpt", {
          message: base_first_prompt,
        });

        const geminiCreateQuestionAndAnswer = [
          { role: "user", content: questionText },
          { role: "assistant", content: base_answer.data },
        ];
        ...
        while (score_count < 3) {
          const refinement = await axios.post("http://localhost:8002/api/gpt", {
            message: second_prompt,
          });
          const jsonString = refinement.data.match(/\{.*\}/s);
          ...
          if (jsonString) {
            ...
            if (similarity_score > 3.0) {
              const third_prompt = `
                ...
                "${presentGeminiQuestion}"\n
              `;
              const refinement_answer = await axios.post(
                "http://localhost:8002/api/gpt",
                {
                  message: third_prompt,
                }
              );

              const geminiCreateQuestionAndAnswer = [
                { role: "user", content: presentGeminiQuestion },
                { role: "assistant", content: refinement_answer.data },
              ];
      ...
      } else if (selectedResponder === "荘司さん") {
        const formattedMessage = `
        タスク:データベースの情報を参照して、入力の答えを回答してください。データベースに回答がない場合は情報を補完し、「データベースに記載がない」は言わなくて結構です。\n
        トピック:農業、沖縄、アップルバナナ、マルエスファーム\n
        話者:あなたは"${selectedResponder}"です。「荘司幸一郎」と自己紹介を最初にしてください。\n
        オーディエンス:"${selectedExplanation}"に説明してください。\n
        長さ:回答は300文字程度にしてください。\n
        出力形式:文章\n
        ##データベース\n
